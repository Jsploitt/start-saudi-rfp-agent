...
Source-grounded asset kit for the Start Saudi proposal-generation system. Built 17 September 2026.

This is the research and content layer that has to exist before any code is written. It is the substitute for the client content library, brand brief and working session that were not available — everything here was extracted from public sources and the supplied brand folder, with provenance recorded throughout.
...
start-saudi-kit/
  ...
  proposal/
    README.md            index + the two findings that change the build
  ...
  OPEN-QUESTIONS.md      Q1–Q34, prioritised, written to be sent as one email
  SOURCES.md             every URL and file opened, and how reliable each was
```

## How to read it

**If you are writing a proposal:** `content/voice.md` first, then `proposal/worked-example.md` to see what "in voice" looks like at full length, then `proposal/section-briefs.md` for the slide you are on. `voice.md` is the file with the highest chance of being skipped and the highest cost of skipping — and now that three real Taajeel proposals are in the corpus, skipping it means writing like the parent company instead.
...
1. **Send the Tier 1 email** at the end of `OPEN-QUESTIONS.md`. Five items, and the first one — the 31-day / 15-working-day contradiction — blocks the timeline slide of every proposal.
2. **Attach the brand guidelines PDF directly** (or just its palette, typography and logo-usage pages as images) → closes Q1, the largest remaining gap.
